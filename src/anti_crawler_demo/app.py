...
DEMOS = {
    ...
    "headless-header": ("無頭模式標頭檢查（Headless header）", "拒絕含 HeadlessChrome 的 User-Agent", "先檢查整頁導覽的 User-Agent；通過後才由頁面 JavaScript 載入資料表。"),
...
TECHNIQUE_INFO = {
    ...
    "headless-header": ("無頭模式 User-Agent 標記檢查", "此頁只讀取 HTTP `User-Agent`。若整頁導覽或頁面後續資料請求的值中包含 `HeadlessChrome`（部分自動化瀏覽器預設會附帶的產品標記），便回傳 403。初始 HTML 不含資料列；通過導覽檢查後，頁面 JavaScript 才會讀取並渲染表格。", "這是觀察標頭差異的教材，不是可靠的身分驗證：User-Agent 可被改寫，且新版 Chrome 的 headless 模式未必含此字串。合規測試應使用正式瀏覽器設定或網站核准的自動化方式；實務上需結合其他伺服器端風險訊號。"),
...
async def action(
    ...
    if demo == "headless-header":
        if is_headless_user_agent(request):
            return reply(request, client, 403, "headless-user-agent", "User-Agent 含有 HeadlessChrome 標記，資料表不會載入。")
        return reply(request, client, 200, "headless-user-agent", "User-Agent 未命中無頭模式標記，資料表已載入。", {"rows": DATASET})
