...
const examples={
 ...
 'headless-header':{cb:`curl -i ${url} -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 HeadlessChrome/140.0 Safari/537.36"`,cp:`curl -s ${url} -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/140.0 Safari/537.36" | grep dataset-table\n# 只取得 HTML 表格殼；資料列由頁面 JavaScript 載入。`,pb:`from playwright.sync_api import sync_playwright\nwith sync_playwright() as p:\n    browser=p.chromium.launch(headless=True)\n    page=browser.new_page()\n    response=page.goto('${url}')\n    print(response.status, response.headers['x-training-rule'])  # 403\n    browser.close()`,pp:`from playwright.sync_api import sync_playwright\nwith sync_playwright() as p:\n    browser=p.chromium.launch(headless=False)\n    page=browser.new_page()\n    response=page.goto('${url}')\n    page.locator('#dataset-table tbody tr').first.wait_for()\n    print(response.status, page.locator('#dataset-table tbody tr').all_inner_texts())\n    browser.close()`},
...
const notes={
 ...
 'headless-header':{
  ...
  'curl-pass':'參數作用：\n- `-s` 只輸出 HTML；`-A` 設定不含 `HeadlessChrome` 的 User-Agent，因此初始頁面可通過唯一規則。\n- `| grep dataset-table` 只證明 curl 拿到表格殼。curl 不執行頁面 JavaScript，無法取得動態資料列；請使用下方 Playwright 範例。\n這是教材用的對照，不能把可修改的 User-Agent 視為正式身分驗證。',
  'python-block':'`p.chromium.launch(headless=True)` 真正啟動 Playwright 的無頭 Chromium；`page.goto()` 回傳該次文件導覽的 HTTP response。此教材版本的 Chromium 會帶 `HeadlessChrome` 標記，因此可直接觀察 403。',
  'python-pass':'`headless=False` 啟動可見 Chromium 視窗，模擬一般使用者的導覽；`locator(...).wait_for()` 確認完整 HTML 的資料列已可讀取後，再輸出表格文字。這不是偽造標頭，而是比較兩種實際瀏覽器模式。'},
 'session-gate':{
